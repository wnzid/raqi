...
export function AddColorwayForm({ source, colors }: { source: AdminProduct; colors: Taxonomy[] }) {
  ...
  return <><BlockingLoader visible={pending} title="Creating colorway…" message={progress} /><form className="mt-8 max-w-3xl space-y-6" onSubmit={submit} aria-busy={pending} inert={pending ? true : undefined}>
    <section className="panel bg-white"><p className="eyebrow">Add colorway to {source.family.name}</p><label className="label mt-5">Color<input className="field" list="colors" value={name} onChange={(event) => setName(event.target.value)} required /></label><datalist id="colors">{colors.map((color) => <option key={color.id} value={color.name} />)}</datalist><p className="mt-3 text-sm"><strong>{source.family.name} | {name || 'Color'}</strong><span className="block muted">/{source.family.slug}-{slugify(name) || 'color'}</span></p><label className="label mt-4">Price (BDT)<input className="field" type="number" min="0.01" step="0.01" value={price} onChange={(event) => setPrice(event.target.value)} required /></label><label className="label mt-4">Sale price (optional)<input className="field" type="number" min="0.01" step="0.01" value={salePrice} onChange={(event) => setSalePrice(event.target.value)} /></label><label className="label mt-4">Images<input className="field" type="file" multiple accept="image/jpeg,image/png,image/webp" onChange={(event) => setFiles(Array.from(event.target.files ?? []))} /></label></section>
