...
function StagedImage({ file, primary, select }: { file: File; primary: boolean; select: () => void }) {
  const [url, setUrl] = useState('');
  useEffect(() => { const next = URL.createObjectURL(file); setUrl(next); return () => URL.revokeObjectURL(next); }, [file]);
  return <figure className={`relative overflow-hidden border bg-[var(--soft)] ${primary ? 'border-black' : 'border-[var(--line)]'}`}><div className="aspect-square">{url && <img className="h-full w-full object-cover" src={url} alt={file.name} />}</div><button className="absolute right-1 top-1 grid h-8 w-8 place-items-center rounded-full bg-white text-lg shadow" type="button" aria-label={primary ? `${file.name} is primary` : `Set ${file.name} as primary image`} title={primary ? 'Primary image' : 'Set as primary'} onClick={select}>{primary ? '★' : '☆'}</button>{primary && <figcaption className="absolute bottom-1 left-1 bg-black px-1.5 py-0.5 text-[.6rem] font-bold tracking-wide text-white">PRIMARY</figcaption>}</figure>;
}
...
export function ProductForm({ brands, colors }: { brands: Taxonomy[]; colors: Taxonomy[] }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setPending(true);
    setError('');
    setProgress('Saving product and colorways…');
    try {
      ...
      setCreated(result);
      const staged = colorways.map((colorway, index) => ({ colorId: resolved[index]!.id, colorName: colorway.name, files: primaryFirst(colorway) }));
      ...
      if (failures.length) {
        ...
        toast.error(message);
      } else { toast.success('Product created successfully.'); router.push('/admin/products'); router.refresh(); }
    } catch (caught) {
  ...
  async function retry() {
    ...
    setPending(true); setError(''); setProgress('Retrying failed uploads…');
    try {
      ...
      else { toast.success('All product images uploaded successfully.'); router.push('/admin/products'); router.refresh(); }
    } finally { setProgress(''); setPending(false); }
  }

  return <>
    <BlockingLoader visible={pending} title={created ? 'Uploading product images…' : 'Creating product…'} message={progress || 'Saving product and colorways…'} />
