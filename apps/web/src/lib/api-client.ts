...
import { secureRandomUuid } from './random-id';
...
export const getActiveAnnouncement = (): Promise<ActiveAnnouncement> => apiRequest('/announcement', (value) => activeAnnouncementSchema.parse(value), { cache: 'no-store' });
export async function listProducts(searchParams: Record<string, string | number | boolean | undefined> = {}): Promise<ProductListResponse> { const query = new URLSearchParams(Object.entries(searchParams).filter((entry): entry is [string, string | number | boolean] => entry[1] !== undefined).map(([key, value]) => [key, String(value)])); return apiRequest(`/products?${query}`, (value) => productListResponseSchema.parse(value), { next: { revalidate: 60, tags: ['catalog', 'homepage-products', 'new-arrivals'] } }); }
export async function getProduct(slug: string): Promise<ProductDetail> { return apiRequest(`/products/slug/${encodeURIComponent(slug)}`, (value) => productDetailSchema.parse(value), { next: { revalidate: 60, tags: ['catalog', `product:${slug}`] } }); }
export async function getRelatedProducts(productId:string):Promise<ProductListResponse['data']>{return apiRequest(`/products/${encodeURIComponent(productId)}/related`,value=>productListResponseSchema.shape.data.parse(value),{next:{revalidate:60,tags:['catalog','related-products',`product:${productId}`]}})}
...
export const checkoutApi = {
  ...
  place: (input: CheckoutInput|Omit<CheckoutInput,'turnstileToken'>,idempotencyKey=secureRandomUuid()): Promise<OrderConfirmation> => apiRequest('/checkout', (value) => orderConfirmationSchema.parse(value), { method: 'POST',headers:{'Idempotency-Key':idempotencyKey}, body: JSON.stringify(input) }),
};
