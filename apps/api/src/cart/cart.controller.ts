...
export class CartController {
  ...
  @Patch('items/:itemId') async update(@Req() req: Request, @Res({ passthrough: true }) res: Response, @Param('itemId') itemId: string, @Body(new ZodValidationPipe(updateCartItemSchema)) input: UpdateCartItemInput) { const total=performance.now(),contextStarted=performance.now(),cartContext=await this.context.resolve(req,res);cartTiming('resolve context',contextStarted);const serviceStarted=performance.now(),result=await this.cart.update(cartContext,itemId,input);cartTiming('cart update service',serviceStarted);cartTiming('PATCH total',total);return result; }
