...
@Injectable() export class AdminCatalogService{
 ...
 async brand(actor:AuthenticatedUser,data:Prisma.BrandCreateInput){const brand=await this.prisma.brand.create({data});await this.audit.write(actor,'BRAND_CREATED','Brand',brand.id,{name:brand.name});return brand}
