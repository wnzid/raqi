...
async function main(){
 ...
 for(const entry of [{color:black,code:'BLK'},{color:white,code:'WHT'}]){
  ...
  for(const[sizeIndex,sizeEu]of[40,41,42].entries()){
   const sku=`URB-${entry.code}-${sizeEu}`,sizeUk=sizeEu-34,sizeUs=sizeEu-33;
   await prisma.productVariant.upsert({where:{productId_colorId_sizeEu_sizeUk_sizeUs:{productId:product.id,colorId:entry.color.id,sizeEu,sizeUk,sizeUs}},update:{sku},create:{productId:product.id,colorId:entry.color.id,sku,sizeEu,sizeUk,sizeUs,stockQuantity:sizeIndex===2&&entry.code==='BLK'?0:8,weightGrams:850}});
  }
 }
}
